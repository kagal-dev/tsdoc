...
describe('resolveDocuments', () => {
  ...
  it('resolves a relative entry input against rootDirectory', () => {
    const fixturesDirectory = path.resolve(__dirname, 'fixtures');

    const manifest = resolveDocuments({
      options: {
        rootDir: fixturesDirectory,
        entries: [{ input: 'sample.ts', name: 'sample' }],
      },
      pkg: { name: '@test/fixture', version: '1.0.0' },
    } as never);
    ...
    expect(entry).toBeDefined();
    expect(entry.symbolCount).toBeGreaterThanOrEqual(3);
    expect(entry.entryFile).toBe('sample.ts');
  });

  it('expands a directory entry to its source files', () => {
    const manifest = resolveDocuments(
      makeContext([
        { input: fixtureDirectory, name: 'dir' },
      ]),
    );

    const entry = manifest.exports['./dir'];
    expect(entry).toBeDefined();
    const names = entry.symbols.map((s) => s.name);
    expect(names).toContain('alpha');
    expect(names).toContain('beta');
    expect(names).not.toContain('excludedTestSymbol');
    expect(names).not.toContain('ExcludedDeclarationSymbol');
    expect(names).not.toContain('excludedTestDirectorySymbol');
  });
...
describe('listSourceFiles', () => {
  it('returns .ts/.tsx files sorted, skipping excluded patterns', () => {
    const files = listSourceFiles(fixtureDirectory)
      .map((f) => path.relative(fixtureDirectory, f));
