...
describe('writeDocuments', () => {
  ...
  it('flattens nested export paths into filenames', () => {
    ...
    const foo = JSON.parse(
      readFileSync(
        path.join(outputDirectory, 'foo-index.json'),
        'utf8',
      ),
    );
    ...
    const bar = JSON.parse(
      readFileSync(
        path.join(outputDirectory, 'bar-index.json'),
        'utf8',
      ),
    );
  ...
  it('throws DuplicateOutputFileError on filename collisions', () => {
    ...
    expect(error.fileName).toBe('foo-index');
    expect(error.previousExportPath).toBe('./foo-index');
    expect(error.conflictingExportPath).toBe('./foo/index');
    expect(error.message).toContain('foo-index.json');
