 ...
 * fills `name` in before hooks fire). Hand-written name lists
 * are rejected by design: only the bundler's own entries carry
 * the effective `stub` flag.
 */
 ...
 * cryptic `TypeError` from the helper reading `rootDir` on
